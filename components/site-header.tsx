...
export function SiteHeader({ context, fixed = false }: SiteHeaderProps) {
  return (
    <nav className={`site-header container-wide z-50 flex items-center justify-between gap-3 bg-cream/95 py-3 backdrop-blur-md sm:gap-6 sm:py-4 ${fixed ? "site-header-fixed" : ""}`} aria-label="Primary navigation">
      ...
      {context ? <span className="site-header-context text-xs font-bold uppercase tracking-[.14em] text-ink/50">{context}</span> : <div className="flex shrink-0 items-center gap-2"><a href={site.whatsapp(defaultWhatsappMessage)} target="_blank" rel="noopener noreferrer" className={buttonStyles("outline", "min-h-10 gap-1.5 px-3 text-xs sm:min-h-11 sm:px-4 sm:text-sm")}><MessageCircle size={15} /><span className="sm:hidden">Consult</span><span className="hidden sm:inline">Consultation</span></a><Link href="/quote" className={buttonStyles("primary", "site-header-cta min-h-10 gap-1.5 px-3 text-xs sm:min-h-11 sm:px-4 sm:text-sm")}><span className="sm:hidden">Estimate</span><span className="hidden sm:inline">Get estimate</span><ArrowUpRight size={15} /></Link></div>}
    </nav>
  );
}
