...
export function SiteHeader({ context, fixed = false }: SiteHeaderProps) {
  ...
  return (
    <nav className={`site-header container-wide z-50 flex items-center justify-between gap-3 bg-cream/95 py-2.5 backdrop-blur-md sm:gap-6 sm:py-3 lg:grid lg:grid-cols-[1fr_auto_1fr] ${fixed ? "site-header-fixed" : ""}`} aria-label="Primary navigation">
      ...
      <div className="site-header-links hidden justify-self-center gap-4 text-md font-semibold text-ink/80 lg:flex">
        {navigationLinks.map(({ label, href }) => {
          const active = isCurrentPath(pathname, href);
          return <a key={href} href={href} aria-current={active ? "page" : undefined}>{label}</a>;
        })}
      </div>
      {context ? <div className="flex shrink-0 items-center gap-2 sm:gap-3 lg:justify-self-end"><span className="site-header-context text-xs font-bold uppercase tracking-[.14em] text-ink/50">{context}</span><MobileNavMenu /></div> : <div className="flex shrink-0 items-center gap-2 sm:gap-3 lg:justify-self-end">
        <MobileNavMenu />
         <a href="/quote" className={buttonStyles("primary", `site-header-cta min-h-9 gap-1 px-2.5 text-[11px] sm:gap-1.5 sm:px-4 sm:text-sm ${isEstimatePage ? "site-header-cta-active" : ""}`)} aria-current={isEstimatePage ? "page" : undefined}><span className="sm:hidden">Estimate</span><span className="hidden sm:inline">Get estimate</span><ArrowUpRight size={14} /></a>
      </div>}
    </nav>
  );
}
