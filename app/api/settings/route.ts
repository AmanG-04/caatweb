...
import { databaseSettingKey, settingsFromRows, TEMPLATE_QUOTE_SETTINGS } from "@/lib/settings";
...
export async function GET(request: Request) {
  ...
  try { return NextResponse.json({ success: true, data: await readSettings() }, { headers: { "Cache-Control": "no-store" } }); } catch { return NextResponse.json({ success: true, data: TEMPLATE_QUOTE_SETTINGS }, { headers: { "Cache-Control": "no-store" } }); }
}
  ...
  if (db) {
    ...
    for (const [key, value] of Object.entries(body)) {
      ...
      await db.prepare("INSERT INTO settings (key,value,value_type,updated_at) VALUES (?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value, value_type=excluded.value_type, updated_at=CURRENT_TIMESTAMP")
        .bind(databaseSettingKey(key), String(value), "number").run();
