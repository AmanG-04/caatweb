...
export async function POST(request: Request) {
  try {
    ...
    if (db) {
      ...
      await db.prepare("INSERT INTO quotes (id,lead_id,engine_version,settings_snapshot,result_json) VALUES (?,?,?,?,?)").bind(quoteId, leadId, "general-band-v1", "{}", JSON.stringify(quote)).run();
    }
