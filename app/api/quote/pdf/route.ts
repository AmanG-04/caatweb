...
type PdfInput = {
  ...
  quote?: { id?: string; systemSizeKw?: number; panelsRequired?: number; roofAreaSqFt?: number; grossCost?: number; subsidy?: number; netCost?: number; monthlySavings?: number; annualSavings?: number; twentyFiveYearSavings?: number; paybackYears?: number; co2OffsetKg?: number };
};
...
function titleCase(value?: string) { return value ? value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()) : "-"; }
...
export async function POST(request: Request) {
  try {
    const input = JSON.parse(await request.text()) as PdfInput;
    ...
    const lines = [
      { value: "CAAT PowerBot LLP", x: 40, y: 805, size: 22, bold: true },
      { value: `SOLAR ESTIMATE`, x: 375, y: 805, size: 9 , bold: true }, { value: 'Generated on: ' + new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata", dateStyle: "medium", timeStyle: "short" }), x: 375, y: 790, size: 7 },
      ...
      { value: [customer.address, customer.city, customer.state, customer.pincode].filter(Boolean).join(", "), x: 48, y: 635, size: 8 },
      ...
      { value: "YOUR INPUTS", x: 48, y: 495, size: 10, bold: true },
      { value: `Energy use: ${customer.monthlyUnits?.toLocaleString("en-IN") ?? "-"} units/month   |   Price: ${customer.pricePerUnit ? pdfInr(customer.pricePerUnit) : "-"}/unit`, x: 48, y: 475, size: 9 },
      { value: `Property: ${titleCase(customer.propertyType)}   |   Roof: ${titleCase(customer.roofType)}   |   Ownership: ${titleCase(customer.ownership)}`, x: 48, y: 458, size: 9 },
      { value: `System preference: ${titleCase(customer.systemType)}   |   Battery: ${customer.batteryRequired === "yes" ? "Required" : "Not required"}${customer.provider ? `   |   Provider: ${customer.provider}` : ""}`, x: 48, y: 441, size: 8 },
