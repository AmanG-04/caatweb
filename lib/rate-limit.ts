...
export async function consumeRateLimit(request: Request, scope: string, limit: number, windowSeconds: number) {
  ...
  try {
    await db.prepare("INSERT INTO rate_limits (key,window_started,request_count) VALUES (?, ?, 1) ON CONFLICT(key) DO UPDATE SET request_count=CASE WHEN window_started < excluded.window_started THEN 1 ELSE request_count+1 END,window_started=CASE WHEN window_started < excluded.window_started THEN excluded.window_started ELSE window_started END,updated_at=CURRENT_TIMESTAMP").bind(key, windowStarted).run();
    ...
    console.error("rate_limit_unavailable", { scope, error: error instanceof Error ? error.message : String(error) });
